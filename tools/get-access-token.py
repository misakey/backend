...
parser.add_argument('--require-account', dest='require_account', action='store_true')
args = parser.parse_args()

# TODO extract error mgmt out of "testContext" because it's not really a test here
with testContext():
    creds = get_credentials(args.email, require_account=args.require_account)
    
print('email:', creds.email)
print('identity_id:', creds.identity_id)
if creds.account_id: print('account_id:', creds.account_id)

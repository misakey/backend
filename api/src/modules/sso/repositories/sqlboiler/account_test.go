...
func testAccountToManyIdentities(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadIdentities(ctx, tx, false, (*[]*Account)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testAccountToManyAddOpIdentities(t *testing.T) {
	var err error
...
var (
	accountDBTypes = map[string]string{`ID`: `uuid`, `Password`: `character varying`}
	_              = bytes.MinRead
)

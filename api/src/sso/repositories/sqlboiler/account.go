...
var AccountRels = struct {
	...
	Identities                   string
	SecretStorageAccountRootKeys string
}{
	BackupArchives:               "BackupArchives",
	CryptoActions:                "CryptoActions",
	Identities:                   "Identities",
	SecretStorageAccountRootKeys: "SecretStorageAccountRootKeys",
}
...
type accountR struct {
	...
	SecretStorageAccountRootKeys SecretStorageAccountRootKeySlice `boil:"SecretStorageAccountRootKeys" json:"SecretStorageAccountRootKeys" toml:"SecretStorageAccountRootKeys" yaml:"SecretStorageAccountRootKeys"`
}
...
func (o *Account) SecretStorageAccountRootKeys(mods ...qm.QueryMod) secretStorageAccountRootKeyQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"secret_storage_account_root_key\".\"account_id\"=?", o.ID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"secret_storage_account_root_key\".*"})
	}
...
// LoadSecretStorageAccountRootKeys allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (accountL) LoadSecretStorageAccountRootKeys(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAccount interface{}, mods queries.Applicator) error {
	var slice []*Account
	var object *Account
	...
	query := NewQuery(
		...
		qm.WhereIn(`secret_storage_account_root_key.account_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load secret_storage_account_root_key")
	}

	var resultSlice []*SecretStorageAccountRootKey
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice secret_storage_account_root_key")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on secret_storage_account_root_key")
	}
	if err = results.Err(); err != nil {
	...
	if singular {
		object.R.SecretStorageAccountRootKeys = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.AccountID {
				local.R.SecretStorageAccountRootKeys = append(local.R.SecretStorageAccountRootKeys, foreign)
				if foreign.R == nil {
					foreign.R = &secretStorageAccountRootKeyR{}
				}
				foreign.R.Account = local
...
// AddSecretStorageAccountRootKeys adds the given related objects to the existing relationships
// of the account, optionally inserting them as new records.
// Appends related to o.R.SecretStorageAccountRootKeys.
// Sets related.R.Account appropriately.
func (o *Account) AddSecretStorageAccountRootKeys(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*SecretStorageAccountRootKey) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.ID, rel.KeyHash}
	...
	if o.R == nil {
		o.R = &accountR{
			SecretStorageAccountRootKeys: related,
		}
	} else {
		o.R.SecretStorageAccountRootKeys = append(o.R.SecretStorageAccountRootKeys, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &secretStorageAccountRootKeyR{
				Account: o,
			}
		} else {
			rel.R.Account = o

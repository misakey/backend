...
func (totpSecretL) LoadIdentity(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTotpSecret interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		foreign.R.TotpSecrets = append(foreign.R.TotpSecrets, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.IdentityID == foreign.ID {
				...
				foreign.R.TotpSecrets = append(foreign.R.TotpSecrets, local)
				break
			}
		}
	}
...
// Adds o to related.R.TotpSecrets.
func (o *TotpSecret) SetIdentity(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Identity) error {
	...
	if related.R == nil {
		related.R = &identityR{
			TotpSecrets: TotpSecretSlice{o},
		}
	} else {
		related.R.TotpSecrets = append(related.R.TotpSecrets, o)
	}
